...
import logging
# set logging to output print statements
...
class UnitTest(ABC):
    ...
    def __call__(self):
        try:
            ...
            if test_passed:
                logging.info(self.define_success_message())
                return 0, 1  # 0 tests failed, 1 total test
            logging.info(self.define_failure_message())
            return 1, 1  # 1 test failed, 1 total test
        except Exception as exception:
            logging.info(self.define_exception_message(exception))
            return 1, 1  # 1 test failed, 1 total test
...
class ConditionedMethodTest(CompositeTest, ABC):
    ...
    def define_success_message(self):
        return "Method %s() using %s correctly implemented." % (self.method_name, self.condition_string)
...
def test_results_to_score(test_results, verbose=True):
    ...
    if verbose:
        logging.info("Score: %d/100" % score)
    return score
